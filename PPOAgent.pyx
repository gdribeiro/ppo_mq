...
class PPOClipped:
    ...
    def createValueNet(self):
        value_net = value_rnn_network.ValueRnnNetwork(
            input_tensor_spec= self.observation_tensor_spec,
            input_fc_layer_params= self.value_fc_layers,
            lstm_size= (128,128),
            dtype=tf.float32,
            output_fc_layer_params= None
        )
...
class PPOAgentMQ:
    ...
    def step(self, _new_state):
        ...
        elif (self.ppo_agent.replay_buffer.num_frames().numpy() < GLOBAL_STEPS/2):
            self.ppo_agent.ppo_agent.train(self.ppo_agent.replay_buffer.gather_all())
